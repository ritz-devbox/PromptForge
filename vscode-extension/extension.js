...
function activate(context) {
    diagnostics = vscode.languages.createDiagnosticCollection('promptforge');
    context.subscriptions.push(diagnostics);

    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument((doc) => {
        if (!isPlanFile(doc.fileName)) {
            return;
        }
        const config = getConfig();
        if (config.lintOnSave) {
            lintPlan(doc.fileName, activePanel);
        }
        if (config.compileOnSave) {
            const projectDir = path.dirname(path.dirname(doc.fileName));
            const outputPath = path.join(projectDir, 'prompt.ir.json');
            compileToJSON(projectDir, outputPath, activePanel, config.explainOnCompile);
        }
    }));

    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument((event) => {
        if (!isPlanFile(event.document.fileName)) {
            return;
        }
        const config = getConfig();
        if (!config.lintOnChange) {
            return;
        }
        const key = event.document.fileName;
        const existing = lintTimers.get(key);
        if (existing) {
            clearTimeout(existing);
        }
        const timer = setTimeout(() => {
            lintPlan(event.document.fileName, activePanel);
        }, 600);
...
`;
        ...
        try {
            ...
            vscode.window.showInformationMessage('✅ Plan created! Opening editor...');
    ...
    let disposable = vscode.commands.registerCommand('promptforge.openEditor', async (uri) => {
        ...
        const config = getConfig();
        if (config.lintOnChange || config.lintOnSave) {
            lintPlan(filePath, panel);
        }
...
function getConfig() {
    ...
    return {
        cliPath: config.get('cliPath', 'promptforge'),
        compileOnSave: config.get('compileOnSave', true),
        lintOnSave: config.get('lintOnSave', true),
        lintOnChange: config.get('lintOnChange', true),
        explainOnCompile: config.get('explainOnCompile', false)
    };
}
...
function isPlanFile(filePath) {
    ...
    return normalized.endsWith('/promptforge/plan.md') || normalized.endsWith('/plan.md');
}

async function lintPlan(filePath, panel) {
    const projectDir = path.dirname(path.dirname(filePath));
    const config = getConfig();
    const cliPath = config.cliPath;

    try {
        const output = await runPromptforge(cliPath, ['lint'], projectDir);
        const diagnosticsList = parseLintOutput(output, filePath);
        const uri = vscode.Uri.file(filePath);
        diagnostics.set(uri, diagnosticsList);
        if (panel && filePath === activePlanPath) {
            postToPanel(panel, {
                command: 'lintResults',
                diagnostics: diagnosticsList.map(diag => ({
                    line: diag.range.start.line + 1,
                    column: diag.range.start.character + 1,
                    severity: diag.severity === vscode.DiagnosticSeverity.Error ? 'error' : 'warn',
                    message: diag.message
                }))
            });
        }
    } catch (error) {
        const message = error.message || 'Lint failed';
        vscode.window.showWarningMessage(message);
        if (panel && filePath === activePlanPath) {
            postToPanel(panel, { command: 'error', message: message });
        }
    }
}

function parseLintOutput(output, filePath) {
    ...
    for (const line of lines) {
        ...
        const severity = severityText === 'error' ? vscode.DiagnosticSeverity.Error : vscode.DiagnosticSeverity.Warning;
...
function runPromptforge(cliPath, args, cwd) {
    return new Promise((resolve, reject) => {
        const cmd = `"${cliPath}" ${args.join(' ')}`;
        exec(cmd, { cwd: cwd, timeout: 30000 }, (error, stdout, stderr) => {
            if (error) {
                const message = stderr && stderr.trim().length > 0 ? stderr : error.message;
    ...
    try {
        ...
        try {
            ...
            if (writeError.code === 'EACCES' || writeError.code === 'EPERM') {
                ...
                postToPanel(panel, { command: 'error', message: errorMsg });
                return;
        ...
        if (config.lintOnSave) {
            await lintPlan(filePath, panel);
        }
        
        // Compile to JSON
        await compileToJSON(projectDir, outputPath, panel, config.explainOnCompile);
...
function compileToJSON(projectDir, outputPath, panel, explain) {
    return new Promise((resolve, reject) => {
        ...
        const promptforgeCmd = getConfig().cliPath || 'promptforge';
        ...
        const compileProcess = exec(`"${promptforgeCmd}" compile${explainFlag}`, { cwd: projectDir, timeout: timeout }, (error, stdout, stderr) => {
            ...
            try {
                if (fs.existsSync(outputPath)) {
                    ...
                    vscode.window.setStatusBarMessage('✅ Plan saved and JSON generated!', 3000);
                    resolve();
                } else {
                    ...
                    if (error) {
                        ...
                        if (error.code === 'ENOENT' || error.message.includes('not found') || error.message.includes('is not recognized')) {
                            ...
                            try {
                                ...
                                vscode.window.setStatusBarMessage('✅ Plan saved! (Using default JSON - CLI not found)', 3000);
                                resolve();
                            } catch (writeError) {
...
<body>
    <div style="margin-bottom: 20px;">
        <h2 style="margin: 0 0 5px 0; color: var(--vscode-textLink-foreground);">🎯 PromptForge</h2>
    ...
    <div class="section">
        <label for="constraints">⚖️ Constraints</label>
    ...
    <div class="buttons">
        <button class="btn-save" onclick="save()">💾 Save & Generate JSON</button>
    </div>

    <div id="status" class="status"></div>

    <div id="lintSection" style="margin-top: 16px; display: none; border: 1px solid var(--vscode-input-border); border-radius: 4px; padding: 12px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <h3 style="margin: 0; color: var(--vscode-textLink-foreground);">Lint Results</h3>
            <span id="lintSummary" style="font-size: 11px; opacity: 0.7;"></span>
        </div>
        <ul id="lintList" style="margin: 0; padding-left: 18px; font-size: 12px;"></ul>
    </div>
    ...
    <div id="jsonSection" style="margin-top: 30px; display: none; border-top: 1px solid var(--vscode-input-border); padding-top: 20px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
            <h3 style="margin: 0; color: var(--vscode-textLink-foreground);">📄 Generated JSON Contract</h3>
        ...
        <div style="margin-top: 12px; display: flex; gap: 10px; align-items: center;">
            <button onclick="copyJSON()" style="padding: 8px 16px; background: var(--vscode-button-background); color: var(--vscode-button-foreground); border: none; border-radius: 4px; cursor: pointer; font-weight: 600;">📋 Copy JSON</button>
            <button onclick="openJSONFile()" style="padding: 8px 16px; background: var(--vscode-button-secondaryBackground); color: var(--vscode-button-secondaryForeground); border: none; border-radius: 4px; cursor: pointer;">📂 Open File</button>
        ...
        <div style="margin-top: 10px; padding: 10px; background: var(--vscode-textBlockQuote-background); border-left: 3px solid var(--vscode-textLink-foreground); border-radius: 2px; font-size: 11px; opacity: 0.9;">
            <strong>💡 Tip:</strong> Copy this JSON and use it with any AI agent or LLM. It contains all the rules, constraints, and failure modes extracted from your plan.
        </div>
    </div>
        ...
        window.addEventListener('message', event => {
            ...
            switch (message.command) {
                case 'jsonGenerated':
                    ...
                    showStatus('?o. JSON generated successfully!', 'success');
                    break;
                case 'error':
                    showStatus('??O ' + message.message, 'error');
                    break;
                case 'lintResults':
                    renderLintResults(message.diagnostics || []);
                    break;
            }
        });

        function renderLintResults(items) {
            ...
            if (!items.length) {
                lintSection.style.display = 'none';
            ...
            items.forEach(item => {
                ...
                li.textContent = `Line ${item.line}:${item.column} ${item.severity.toUpperCase()} ${item.message}`;
            ...
            lintSummary.textContent = `${errorCount} error(s), ${warnCount} warning(s)`;
            lintSection.style.display = 'block';
        }
